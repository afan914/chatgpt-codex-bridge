...
当前只支持 Google Chrome 或可通过 `chrome://extensions` 加载未打包 Manifest V3 扩展的 Chromium 浏览器。

ChatGPT Atlas 不能安装或运行这个浏览器插件，因此在 Atlas 里无法使用本项目的扩展能力。
...
在 Chrome 的 `chrome://extensions` 里加载 `apps/extension/dist`。ChatGPT Atlas 不能加载这个扩展。
