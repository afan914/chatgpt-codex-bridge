...
对话内容由 content script 从当前 ChatGPT 页面读取。导出上下文包时，内容会在扩展 background service worker 内本地生成 zip 并由浏览器下载。导入 Codex 项目时，内容才会发送到 `127.0.0.1` 上的本地 Bridge，Bridge 再写入配置好的本地项目目录。本工具不会把对话内容上传到远程服务器。
...
只有直接导入 Codex 项目时，用户才需要显式配置项目目录：
